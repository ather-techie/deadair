...
def test_show_transcript_runs_transcribe_step_even_without_remove_filler(client):
    upload = client.post(
        "/api/videos",
        files={"video": ("clip.mp4", io.BytesIO(b"data"), "video/mp4")},
        data={"remove_silence": "true", "remove_filler": "false", "show_transcript": "true"},
    )
...
def test_transcript_endpoint_409s_when_not_requested(client):
    upload = client.post(
        "/api/videos",
        files={"video": ("clip.mp4", io.BytesIO(b"data"), "video/mp4")},
        data={"remove_silence": "true", "remove_filler": "false", "show_transcript": "false"},
    )
    video_id = upload.json()["video_id"]
